	...
	class WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( 0 === strcasecmp( $item->attr_title, 'divider' ) && 1 === $depth ) {
				...
				// Depth parent link wroking condition
				...
				$item_output .= ( isset( $args->has_children ) && $args->has_children && 0 === $depth ) ? '  <i class="las la-angle-down"></i></a>' : '</a>';
		...
		public static function fallback( $args ) {
			if ( current_user_can( 'edit_theme_options' ) ) {
				...
				echo '>';
				echo '<li><a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '">' . esc_attr( 'Add a menu', 'welearner' ) . '</a></li>';
				echo '</ul>';
				if ( $container ) {
					echo '</' . esc_attr( $container ) . '>'; }
			}
